...
async function post(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (valid === false) {
    throw new BadReqErr('요청이 잘못 되었습니다.');
  }

  const { uid, message, author } = req.body;
  if (uid === undefined) {
    throw new BadReqErr('uid 누락');
  }
  if (message === undefined) {
    throw new BadReqErr('message 누락');
  }
...
async function get(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (messageId === undefined) {
    throw new BadReqErr('messageId 누락');
  }
  const uidToStr = Array.isArray(uid) ? uid[0] : uid;
...
async function updateMessage(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    throw new BadReqErr('token에 문제가 있습니다');
  }
